...
func ExampleTracer() {
	...
	// ATATUS_SERVER_URL should typically set in the environment
	...
	tracer, err := atatus.NewTracer(serviceName, serviceVersion)
	if err != nil {
		log.Fatal(err)
	}
	defer tracer.Close()
	...
	// (ATATUS_API_REQUEST_SIZE) or a given duration has
	// elapsed (ATATUS_API_REQUEST_TIME). Even so, we flush
...
func (api *api) handleOrder(ctx context.Context, product string) {
	...
	defer tx.End()
	ctx = atatus.ContextWithTransaction(ctx, tx)
...
func storeOrder(ctx context.Context, product string) {
	span, _ := atatus.StartSpan(ctx, "store_order", "rpc")
	defer span.End()
