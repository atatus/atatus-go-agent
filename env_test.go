...
func TestTracerRequestTimeEnv(t *testing.T) {
	os.Setenv("ATATUS_API_REQUEST_TIME", "1s")
	defer os.Unsetenv("ATATUS_API_REQUEST_TIME")
	...
	os.Setenv("ATATUS_SERVER_URL", server.URL)
	defer os.Unsetenv("ATATUS_SERVER_URL")
...
func TestTracerRequestTimeEnvInvalid(t *testing.T) {
	t.Run("invalid_duration", func(t *testing.T) {
		os.Setenv("ATATUS_API_REQUEST_TIME", "aeon")
		defer os.Unsetenv("ATATUS_API_REQUEST_TIME")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "failed to parse ATATUS_API_REQUEST_TIME: invalid duration aeon")
	})
	t.Run("missing_suffix", func(t *testing.T) {
		os.Setenv("ATATUS_API_REQUEST_TIME", "1")
		defer os.Unsetenv("ATATUS_API_REQUEST_TIME")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "failed to parse ATATUS_API_REQUEST_TIME: missing unit in duration 1 (allowed units: ms, s, m)")
	})
}

func TestTracerRequestSizeEnvInvalid(t *testing.T) {
	t.Run("too_small", func(t *testing.T) {
		os.Setenv("ATATUS_API_REQUEST_SIZE", "1B")
		defer os.Unsetenv("ATATUS_API_REQUEST_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_API_REQUEST_SIZE must be at least 1KB and less than 5MB, got 1B")
	})
	t.Run("too_large", func(t *testing.T) {
		os.Setenv("ATATUS_API_REQUEST_SIZE", "500GB")
		defer os.Unsetenv("ATATUS_API_REQUEST_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_API_REQUEST_SIZE must be at least 1KB and less than 5MB, got 500GB")
	})
}

func TestTracerBufferSizeEnvInvalid(t *testing.T) {
	t.Run("too_small", func(t *testing.T) {
		os.Setenv("ATATUS_API_BUFFER_SIZE", "1B")
		defer os.Unsetenv("ATATUS_API_BUFFER_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_API_BUFFER_SIZE must be at least 10KB and less than 100MB, got 1B")
	})
	t.Run("too_large", func(t *testing.T) {
		os.Setenv("ATATUS_API_BUFFER_SIZE", "500GB")
		defer os.Unsetenv("ATATUS_API_BUFFER_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_API_BUFFER_SIZE must be at least 10KB and less than 100MB, got 500GB")
	})
}

func TestTracerMetricsBufferSizeEnvInvalid(t *testing.T) {
	t.Run("too_small", func(t *testing.T) {
		os.Setenv("ATATUS_METRICS_BUFFER_SIZE", "1B")
		defer os.Unsetenv("ATATUS_METRICS_BUFFER_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_METRICS_BUFFER_SIZE must be at least 10KB and less than 100MB, got 1B")
	})
	t.Run("too_large", func(t *testing.T) {
		os.Setenv("ATATUS_METRICS_BUFFER_SIZE", "500GB")
		defer os.Unsetenv("ATATUS_METRICS_BUFFER_SIZE")
		_, err := atatus.NewTracer("tracer_testing", "")
		assert.EqualError(t, err, "ATATUS_METRICS_BUFFER_SIZE must be at least 10KB and less than 100MB, got 500GB")
	})
}
...
func TestTracerTransactionRateEnvInvalid(t *testing.T) {
	os.Setenv("ATATUS_TRANSACTION_SAMPLE_RATE", "2.0")
	defer os.Unsetenv("ATATUS_TRANSACTION_SAMPLE_RATE")

	_, err := atatus.NewTracer("tracer_testing", "")
	assert.EqualError(t, err, "invalid value for ATATUS_TRANSACTION_SAMPLE_RATE: 2.0 (out of range [0,1.0])")
}

func testTracerTransactionRateEnv(t *testing.T, envValue string, ratio float64) {
	os.Setenv("ATATUS_TRANSACTION_SAMPLE_RATE", envValue)
	defer os.Unsetenv("ATATUS_TRANSACTION_SAMPLE_RATE")
...
func testTracerSanitizeFieldNamesEnv(t *testing.T, envValue, expect string) {
	os.Setenv("ATATUS_SANITIZE_FIELD_NAMES", envValue)
	defer os.Unsetenv("ATATUS_SANITIZE_FIELD_NAMES")
...
func TestTracerServiceNameEnvSanitizationSpecified(t *testing.T) {
	_, _, service, _ := getSubprocessMetadata(t, "ATATUS_SERVICE_NAME=foo!bar")
...
func TestTracerGlobalLabelsSpecified(t *testing.T) {
	_, _, _, labels := getSubprocessMetadata(t, "ATATUS_GLOBAL_LABELS=a=b,c = d")
...
func TestTracerCaptureBodyEnvInvalid(t *testing.T) {
	os.Setenv("ATATUS_CAPTURE_BODY", "invalid")
	defer os.Unsetenv("ATATUS_CAPTURE_BODY")
	_, err := atatus.NewTracer("", "")
	assert.EqualError(t, err, `invalid ATATUS_CAPTURE_BODY value "invalid"`)
}

func testTracerCaptureBodyEnv(t *testing.T, envValue string, expectBody bool) {
	os.Setenv("ATATUS_CAPTURE_BODY", envValue)
	defer os.Unsetenv("ATATUS_CAPTURE_BODY")
...
func TestTracerSpanFramesMinDurationEnv(t *testing.T) {
	os.Setenv("ATATUS_SPAN_FRAMES_MIN_DURATION", "10ms")
	defer os.Unsetenv("ATATUS_SPAN_FRAMES_MIN_DURATION")
...
func TestTracerSpanFramesMinDurationEnvInvalid(t *testing.T) {
	os.Setenv("ATATUS_SPAN_FRAMES_MIN_DURATION", "aeon")
	defer os.Unsetenv("ATATUS_SPAN_FRAMES_MIN_DURATION")

	_, err := atatus.NewTracer("tracer_testing", "")
	assert.EqualError(t, err, "failed to parse ATATUS_SPAN_FRAMES_MIN_DURATION: invalid duration aeon")
}

func TestTracerStackTraceLimitEnv(t *testing.T) {
	os.Setenv("ATATUS_STACK_TRACE_LIMIT", "0")
	defer os.Unsetenv("ATATUS_STACK_TRACE_LIMIT")
...
func TestTracerStackTraceLimitEnvInvalid(t *testing.T) {
	os.Setenv("ATATUS_STACK_TRACE_LIMIT", "sky")
	defer os.Unsetenv("ATATUS_STACK_TRACE_LIMIT")

	_, err := atatus.NewTracer("tracer_testing", "")
	assert.EqualError(t, err, "failed to parse ATATUS_STACK_TRACE_LIMIT: strconv.Atoi: parsing \"sky\": invalid syntax")
}

func TestTracerActiveEnv(t *testing.T) {
	os.Setenv("ATATUS_ACTIVE", "false")
	defer os.Unsetenv("ATATUS_ACTIVE")
...
func TestTracerActiveEnvInvalid(t *testing.T) {
	os.Setenv("ATATUS_ACTIVE", "yep")
	defer os.Unsetenv("ATATUS_ACTIVE")

	_, err := atatus.NewTracer("tracer_testing", "")
	assert.EqualError(t, err, "failed to parse ATATUS_ACTIVE: strconv.ParseBool: parsing \"yep\": invalid syntax")
}

func TestTracerEnvironmentEnv(t *testing.T) {
	os.Setenv("ATATUS_ENVIRONMENT", "friendly")
	defer os.Unsetenv("ATATUS_ENVIRONMENT")
...
func TestTracerCaptureHeadersEnv(t *testing.T) {
	os.Setenv("ATATUS_CAPTURE_HEADERS", "false")
	defer os.Unsetenv("ATATUS_CAPTURE_HEADERS")
...
func TestServiceNodeNameEnvSpecified(t *testing.T) {
	_, _, service, _ := getSubprocessMetadata(t, "ATATUS_SERVICE_NODE_NAME=foo_bar")
	assert.Equal(t, "foo_bar", service.Node.ConfiguredName)
}

func TestUseAtatusTraceparentHeader(t *testing.T) {
	t.Run("default", func(t *testing.T) { testUseAtatusTraceparentHeader(t, "", true) })
	t.Run("false", func(t *testing.T) { testUseAtatusTraceparentHeader(t, "false", false) })
	t.Run("true", func(t *testing.T) { testUseAtatusTraceparentHeader(t, "true", true) })
}

func testUseAtatusTraceparentHeader(t *testing.T, envValue string, expectPropagate bool) {
	os.Setenv("ATATUS_USE_TRACEPARENT_HEADER", envValue)
	defer os.Unsetenv("ATATUS_USE_TRACEPARENT_HEADER")
