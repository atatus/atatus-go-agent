...
import (
	...
	"go.atatus.com/agent/internal/apmschema"
)

func TestValidateServiceName(t *testing.T) {
	validatePayloadMetadata(t, func(tracer *atatus.Tracer) {
...
func TestValidateTransactionName(t *testing.T) {
	validatePayloads(t, func(tracer *atatus.Tracer) {
...
func TestValidateSpanName(t *testing.T) {
	validateTransaction(t, func(tx *atatus.Transaction) {
...
func TestValidateDatabaseSpanContext(t *testing.T) {
	validateSpan(t, func(s *atatus.Span) {
		s.Context.SetDatabase(atatus.DatabaseSpanContext{
	...
	for _, name := range []string{"", overlong} {
		for _, resource := range []string{"", overlong} {
			validateSpan(t, func(s *atatus.Span) {
				...
				s.Context.SetDestinationService(atatus.DestinationServiceSpanContext{
					Name:     name,
					Resource: resource,
				})
			})
...
func TestValidateErrorLog(t *testing.T) {
	tests := map[string]atatus.ErrorLogRecord{
		"empty_message": {
			Message: "",
		},
		"long_message": {
...
func TestValidateMetrics(t *testing.T) {
	gather := func(ctx context.Context, m *atatus.Metrics) error {
		m.Add("without_labels", nil, -66)
		m.Add("with_labels", []atatus.MetricLabel{
	...
	validatePayloads(t, func(tracer *atatus.Tracer) {
		unregister := tracer.RegisterMetricsGatherer(atatus.GatherMetricsFunc(gather))
		defer unregister()
		tracer.SendMetrics(nil)
	})
}

func validateSpan(t *testing.T, f func(s *atatus.Span)) {
	validateTransaction(t, func(tx *atatus.Transaction) {
...
func validateTransaction(t *testing.T, f func(tx *atatus.Transaction)) {
	validatePayloads(t, func(tracer *atatus.Tracer) {
		tx := tracer.StartTransaction("name", "type")
		f(tx)
		tx.End()
...
func validatePayloadMetadata(t *testing.T, f func(tracer *atatus.Tracer)) {
	validatePayloads(t, func(tracer *atatus.Tracer) {
		f(tracer)
...
func validatePayloads(t *testing.T, f func(tracer *atatus.Tracer)) {
	tracer, _ := atatus.NewTracerOptions(atatus.TracerOptions{
		ServiceName:        "x",
		ServiceVersion:     "y",
