...
type TraceState struct {
	...
	// Fields related to parsing the Atatus ("at") tracestate entry.
	//
	// These must not be modified after NewTraceState returns.
	parseAtatusTracestateError error
	haveSampleRate             bool
	sampleRate                 float64
}
...
func NewTraceState(entries ...TraceStateEntry) TraceState {
	...
	for _, e := range entries {
		if e.Key == atatusTracestateVendorKey {
			if haveAtatus {
	...
	if haveAtatus {
		out.parseAtatusTracestateError = out.parseAtatusTracestate(*out.head)
	}
	return out
}

// parseAtatusTracestate parses an Atatus ("at") tracestate entry.
//
// the "at" tracestate value format is: "key:value;key:value...". Unknown keys are ignored.
func (s *TraceState) parseAtatusTracestate(e TraceStateEntry) error {
	...
	for value != "" {
		...
		if sep == -1 {
			return errors.New("malformed 'at' tracestate entry")
		}
		k, v := kv[:sep], kv[sep+1:]
		switch k {
		case "s":
...
func (s TraceState) Validate() error {
	...
	for e := s.head; e != nil; e = e.next {
		...
		if e.Key == atatusTracestateVendorKey {
			// s.parseAtatusTracestateError holds a general e.Validate error if any
			// occurred, or any other error specific to the Elastic tracestate format.
			if err := s.parseAtatusTracestateError; err != nil {
...
func formatAtatusTracestateValue(sampleRate float64) string {
	// 0       -> "s:0"
	// 1       -> "s:1"
