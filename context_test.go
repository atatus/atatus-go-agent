...
func TestContextLabels(t *testing.T) {
	type customInt int
	tx := testSendTransaction(t, func(tx *atatus.Transaction) {
...
func testSendTransaction(t *testing.T, f func(tx *atatus.Transaction)) model.Transaction {
	transaction, _, _ := apmtest.WithTransaction(func(ctx context.Context) {
		f(atatus.TransactionFromContext(ctx))
	})
	return transaction
}
