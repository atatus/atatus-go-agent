...
import (
	...
	"go.atatus.com/agent/internal/iochan"
...
type TracerOptions struct {
	...
	// ATATUS_SERVICE_NAME environment variable, or if that is not set,
	// the executable name.
	ServiceName string
	...
	// the ATATUS_SERVICE_VERSION environment variable.
	...
	// using the ATATUS_ENVIRONMENT environment variable.
	...
	// the environment variable ATATUS_CENTRAL_CONFIG=false.
...
func (opts *TracerOptions) initDefaults(continueOnError bool) error {
	...
	propagateLegacyHeader, err := initialUseAtatusTraceparentHeader()
	if failed(err) {
...
func newTracer(opts TracerOptions) *Tracer {
	...
	t.setLocalInstrumentationConfig(envUseAtatusTraceparentHeader, func(cfg *instrumentationConfigValues) {
...
// no transactions or errors will be sent to the Atatus server.
...
// propagate the legacy "Atatus-Apm-Traceparent" header in addition to
...
// environment variables ATATUS_LOG_FILE and ATATUS_LOG_LEVEL.
...
func (t *Tracer) loop() {
	...
	for {
		...
		case event := <-t.events:
			...
			case spanEvent:
				agg.c.spanChan <- event
				// modelWriter.writeSpan(event.span.Span, event.span.SpanData)
			case errorEvent:
				// var out model.Error
				// modelWriter.buildModelError(&out, event.err)
				agg.c.errChan <- event
