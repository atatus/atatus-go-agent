...
import (
	...
	"go.atatus.com/agent/apmconfig"
	"go.atatus.com/agent/transport"
)

func init() {
	// Don't let the environment influence tests.
	os.Unsetenv("ATATUS_SERVER_TIMEOUT")
	os.Unsetenv("ATATUS_SERVER_URLS")
	os.Unsetenv("ATATUS_SERVER_URL")
	os.Unsetenv("ATATUS_SECRET_TOKEN")
	os.Unsetenv("ATATUS_SERVER_CERT")
	os.Unsetenv("ATATUS_VERIFY_SERVER_CERT")
}
...
func TestHTTPTransportUserAgent(t *testing.T) {
	...
	defer server.Close()
	defer patchEnv("ATATUS_SERVER_URL", server.URL)()
	...
	assert.Regexp(t, "atatus-go/.* go/.*", h.requests[0].UserAgent())
...
func TestHTTPTransportEnvSecretToken(t *testing.T) {
	...
	defer server.Close()
	defer patchEnv("ATATUS_SERVER_URL", server.URL)()
	defer patchEnv("ATATUS_SECRET_TOKEN", "hunter2")()
...
func TestHTTPTransportEnvAPIKey(t *testing.T) {
	...
	defer server.Close()
	defer patchEnv("ATATUS_SERVER_URL", server.URL)()
	defer patchEnv("ATATUS_API_KEY", "api_key_wins")()
	defer patchEnv("ATATUS_SECRET_TOKEN", "secret_token_loses")()
...
func TestHTTPTransportEnvVerifyServerCert(t *testing.T) {
	...
	defer server.Close()
	defer patchEnv("ATATUS_SERVER_URL", server.URL)()
	defer patchEnv("ATATUS_VERIFY_SERVER_CERT", "false")()
...
func TestHTTPTransportServerTimeout(t *testing.T) {
	...
	defer close(done)
	defer patchEnv("ATATUS_SERVER_URL", server.URL)()
	defer patchEnv("ATATUS_SERVER_TIMEOUT", "50ms")()
...
func TestHTTPTransportCACert(t *testing.T) {
	...
	defer patchEnv("ATATUS_SERVER_CA_CERT_FILE", "./testdata/file_that_doesnt_exist.pem")()
	trans, err = transport.NewHTTPTransport()
	assert.Error(t, err)
	assert.Nil(t, trans)
	...
	defer f.Close()
	defer patchEnv("ATATUS_SERVER_CA_CERT_FILE", f.Name())()
	trans, err = transport.NewHTTPTransport()
	assert.Error(t, err)
	assert.Nil(t, trans)

	// Set a certificate that doesn't match, SendStream should still fail
	defer patchEnv("ATATUS_SERVER_CA_CERT_FILE", "./testdata/cert.pem")()
	trans, err = transport.NewHTTPTransport()
	assert.NoError(t, err)
	assert.NotNil(t, trans)
	err = trans.SendStream(context.Background(), p)
...
func TestHTTPTransportServerCert(t *testing.T) {
	...
	// Set a certificate that doesn't match, SendStream should still fail.
	defer patchEnv("ATATUS_SERVER_CERT", "./testdata/cert.pem")()
	transport = newTransport()
	err = transport.SendStream(context.Background(), p)
	assert.Error(t, err)
	...
	defer os.Remove(f.Name())
	defer f.Close()
	defer patchEnv("ATATUS_SERVER_CERT", f.Name())()
