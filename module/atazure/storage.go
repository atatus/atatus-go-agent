...
package atazure // import "go.atatus.com/agent/module/atazure"
...
func (p *apmPipeline) Do(
	...
	if rpc._type() == "messaging" && (req.Method == "GET" || req.Method == "") {
		...
		req.Request = r
		defer tx.End()
		span = tx.StartSpan(rpc.name(), rpc._type(), atatus.SpanFromContext(ctx))
	} else {
		span, ctx = atatus.StartSpan(ctx, rpc.name(), rpc._type())
	}

	defer span.End()
	if !span.Dropped() {
		ctx = atatus.ContextWithSpan(ctx, span)
		req.Request = athttp.RequestWithContext(ctx, req.Request)
