...
package atnegroni // import "go.atatus.com/agent/module/atnegroni"
...
func Middleware(o ...Option) negroni.Handler {
	m := &middleware{
		handler: athttp.Wrap(http.HandlerFunc(nextHandler), athttpServerOptions(o...)...),
	}
	return m
}
...
type Option athttp.ServerOption

func athttpServerOptions(o ...Option) []athttp.ServerOption {
	opts := make([]athttp.ServerOption, len(o))
	for i, opt := range o {
		opts[i] = athttp.ServerOption(opt)
	}
	return opts
}

// RecoveryFunc is the type of a function for use in WithRecovery.
type RecoveryFunc athttp.RecoveryFunc

// RequestNameFunc is the type of a function for use in
// WithServerRequestName.
type RequestNameFunc athttp.RequestNameFunc
...
func WithServerRequestName(r RequestNameFunc) Option {
	return Option(athttp.WithServerRequestName(athttp.RequestNameFunc(r)))
}

// WithTracer returns a Option which sets t as the tracer
// to use for tracing server requests.
func WithTracer(t *atatus.Tracer) Option {
	return Option(athttp.WithTracer(t))
}
...
func WithRecovery(r RecoveryFunc) Option {
	return Option(athttp.WithRecovery(athttp.RecoveryFunc(r)))
}
...
func WithPanicPropagation() Option {
	return Option(athttp.WithPanicPropagation())
}

// RequestIgnorerFunc is the type of a function for use in
// WithServerRequestIgnorer.
type RequestIgnorerFunc athttp.RequestIgnorerFunc
...
func WithServerRequestIgnorer(r RequestIgnorerFunc) Option {
	return Option(athttp.WithServerRequestIgnorer(athttp.RequestIgnorerFunc(r)))
}
