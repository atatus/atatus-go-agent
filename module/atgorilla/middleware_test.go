...
import (
	...
	"go.atatus.com/agent/module/atgorilla"
)
...
func TestMuxMiddleware(t *testing.T) {
	...
	r := mux.NewRouter()
	r.Use(atgorilla.Middleware(atgorilla.WithTracer(tracer.Tracer)))
...
func TestInstrumentUnknownRoute(t *testing.T) {
	...
	r := mux.NewRouter()
	atgorilla.Instrument(r, atgorilla.WithTracer(tracer.Tracer))
...
func TestInstrumentWithPanicPropagation(t *testing.T) {
	for _, propagate := range []bool{false, true} {
		t.Run(fmt.Sprint(propagate), func(t *testing.T) {
			opts := []atgorilla.Option{
				atgorilla.WithTracer(apmtest.DiscardTracer),
			}
			assertPanics := assert.NotPanics
			if propagate {
				opts = append(opts, atgorilla.WithPanicPropagation())
				assertPanics = assert.Panics
			}
			r := mux.NewRouter()
			atgorilla.Instrument(r, opts...)
