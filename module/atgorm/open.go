...
// atgorm/dialects package has been imported (or the driver has
// otherwise been registered with atsql), then the datasource name
...
func Open(dialect string, args ...interface{}) (*gorm.DB, error) {
	...
	registerCallbacks(db, atsql.DriverDSNParser(driverName)(dsn))
	return db, nil
}
