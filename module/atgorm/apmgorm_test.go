...
import (
	...
	_ "go.atatus.com/agent/module/atgorm/dialects/sqlite"
...
func testWithContext(t *testing.T, dsnInfo atsql.DSNInfo, dialect string, args ...interface{}) {
	_, spans, errors := apmtest.WithTransaction(func(ctx context.Context) {
		db, err := atgorm.Open(dialect, args...)
...
func TestWithContextNoTransaction(t *testing.T) {
	db, err := atgorm.Open("sqlite3", ":memory:")
	require.NoError(t, err)
	defer db.Close()
	db = atgorm.WithContext(context.Background(), db)
...
func TestWithContextNonSampled(t *testing.T) {
	os.Setenv("ATATUS_TRANSACTION_SAMPLE_RATE", "0")
...
func TestCaptureErrors(t *testing.T) {
	...
	if os.Getenv("PGHOST") == "" {
		...
		t.Run("postgres", func(t *testing.T) {
			db, err := atgorm.Open("postgres", "user=postgres password=hunter2 dbname=test_db sslmode=disable")
			require.NoError(t, err)
			defer db.Close()
			testCaptureErrors(t, db)
		})
...
func TestOpenWithDriver(t *testing.T) {
	db, err := atgorm.Open("sqlite3", "sqlite3", ":memory:")
	require.NoError(t, err)
	defer db.Close()
	db.DropTableIfExists(&Product{})
	db.AutoMigrate(&Product{})
...
func TestOpenWithDB(t *testing.T) {
	...
	db, err := atgorm.Open("sqlite3", sqldb)
	require.NoError(t, err)
	defer db.Close()
	db.DropTableIfExists(&Product{})
	db.AutoMigrate(&Product{})
