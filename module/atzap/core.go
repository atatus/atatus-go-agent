...
package atzap // import "go.atatus.com/agent/module/atzap"
...
func (c *Core) tracer() *atatus.Tracer {
	tracer := c.Tracer
	if tracer == nil {
...
func (c *traceContext) fields(fields []zapcore.Field) {
	for _, field := range fields {
		...
		case FieldKeyTraceID:
			c.traceID, _ = field.Interface.(atatus.TraceID)
		case FieldKeyTransactionID:
			c.transactionID, _ = field.Interface.(atatus.SpanID)
		case FieldKeySpanID:
			c.spanID, _ = field.Interface.(atatus.SpanID)
		}
	}
}
