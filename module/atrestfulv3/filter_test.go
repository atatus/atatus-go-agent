...
import (
	...
	"go.atatus.com/agent/module/atrestfulv3"
...
func TestHandlerHTTPSuite(t *testing.T) {
	...
	ws.Route(ws.POST("/explicit_error_capture").To(func(req *restful.Request, resp *restful.Response) {
		...
		e := atatus.CaptureError(req.Request.Context(), errors.New("total explosion"))
		e.Send()
	...
	container.Add(&ws)
	container.Filter(atrestfulv3.Filter(atrestfulv3.WithTracer(tracer)))
...
func TestContainerFilter(t *testing.T) {
	...
	ws.Route(ws.GET("/{id:[0-1]+}").To(func(req *restful.Request, resp *restful.Response) {
		if atatus.TransactionFromContext(req.Request.Context()) == nil {
