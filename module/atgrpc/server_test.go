...
import (
	...
	"go.atatus.com/agent/module/atgrpc/internal/testservice"
...
func testServerTransactionHappy(t *testing.T, p testParams) {
	traceID := atatus.TraceID{0x0a, 0xf7, 0x65, 0x19, 0x16, 0xcd, 0x43, 0xdd, 0x84, 0x48, 0xeb, 0x21, 0x1c, 0x80, 0x31, 0x9c}
	clientSpanID := atatus.SpanID{0xb7, 0xad, 0x6b, 0x71, 0x69, 0x20, 0x33, 0x31}

	traceparentValue := fmt.Sprintf("00-%s-%s-01", traceID, clientSpanID)

	headers := []string{athttp.AtatusTraceparentHeader, athttp.W3CTraceparentHeader}
		...
		assert.Equal(t, &model.Context{
			...
			Request: &model.Request{
				...
				Headers: model.Headers{{
					...
					Key:    "user-agent",
					Values: []string{"atgrpc_test grpc-go/" + grpc.Version},
				}},
...
func TestServerRecovery(t *testing.T) {
	...
	s, server, addr := newGreeterServer(t, tracer, atgrpc.WithRecovery())
	defer s.GracefulStop()
...
func TestServerIgnorer(t *testing.T) {
	...
	s, _, addr := newGreeterServer(t, tracer, atgrpc.WithRecovery(), atgrpc.WithServerRequestIgnorer(func(*grpc.UnaryServerInfo) bool {
		return true
	}))
	defer s.GracefulStop()
...
func TestServerStream(t *testing.T) {
	...
	s, accumulatorServer, addr := newAccumulatorServer(t, tracer, atgrpc.WithRecovery())
	defer s.GracefulStop()
...
func newGreeterServer(t *testing.T, tracer *atatus.Tracer, opts ...atgrpc.ServerOption) (*grpc.Server, *helloworldServer, net.Addr) {
	return newGreeterServerTLS(t, tracer, nil, opts...)
}

func newGreeterServerTLS(t *testing.T, tracer *atatus.Tracer, tlsConfig *tls.Config, opts ...atgrpc.ServerOption) (*grpc.Server, *helloworldServer, net.Addr) {
...
func newGreeterClient(t *testing.T, addr net.Addr) (*grpc.ClientConn, pb.GreeterClient) {
	conn, err := grpc.Dial(
		...
		grpc.WithUserAgent("atgrpc_test"),
	)
	require.NoError(t, err)
...
func newAccumulatorServer(t *testing.T, tracer *atatus.Tracer, opts ...atgrpc.ServerOption) (*grpc.Server, *accumulator, net.Addr) {
	// We always install grpc_recovery first to avoid panics
	// aborting the test process. We install it before the
	// atgrpc interceptor so that atgrpc can recover panics
	// itself if configured to do so.
	...
	if tracer != nil {
		opts = append(opts, atgrpc.WithTracer(tracer))
		interceptors = append(interceptors, atgrpc.NewStreamServerInterceptor(opts...))
	}
...
func (s *helloworldServer) SayHello(ctx context.Context, req *pb.HelloRequest) (*pb.HelloReply, error) {
	...
	if md, ok := metadata.FromIncomingContext(ctx); ok {
		...
		for _, header := range []string{"atatus-apm-traceparent", "traceparent", "tracestate"} {
			if values := md.Get(header); len(values) > 0 {
				tx.Context.SetCustom(header, strings.Join(values, " "))
			}
		}
	}
	span, ctx := atatus.StartSpan(ctx, "server_span", "type")
...
type accumulator struct {
	...
	transactionFromContext *atatus.Transaction
}

func (a *accumulator) Accumulate(srv testservice.Accumulator_AccumulateServer) error {
	a.transactionFromContext = atatus.TransactionFromContext(srv.Context())
	if a.panic {
		panic(a.err)
	}
	if a.err != nil {
