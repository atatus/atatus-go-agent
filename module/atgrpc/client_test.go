...
func TestClientSpan(t *testing.T) {
	t.Run("with-atatus-apm-traceparent", func(t *testing.T) {
		testClientSpan(t, "atatus-apm-traceparent", "traceparent")
	})
	t.Run("without-atatus-apm-traceparent", func(t *testing.T) {
		os.Setenv("ATATUS_USE_TRACEPARENT_HEADER", "false")
...
func testClientSpan(t *testing.T, traceparentHeaders ...string) {
	...
	_, clientSpans, _ := apmtest.WithTransactionOptions(atatus.TransactionOptions{
	...
	traceparentValue := athttp.FormatTraceparentHeader(atatus.TraceContext{
		Trace:   atatus.TraceID(clientSpans[0].TraceID),
		Span:    atatus.SpanID(clientSpans[0].ID),
		Options: atatus.TraceOptions(0).WithRecorded(true),
	})
...
func TestClientTransactionUnsampled(t *testing.T) {
	...
	clientTracer := apmtest.NewRecordingTracer()
	defer clientTracer.Close()
	clientTracer.SetSampler(atatus.NewRatioSampler(0)) // sample nothing
...
func TestStreamClientSpan(t *testing.T) {
	...
	ctx := atatus.ContextWithTransaction(context.Background(), clientTransaction)
	...
	assert.Equal(t, "/go.elastic.co.atatus.module.atgrpc.testservice.Accumulator/Accumulate", clientPayloads.Spans[0].Name)
