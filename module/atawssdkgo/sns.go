...
func addMessageAttributesSNS(req *request.Request, span *atatus.Span, propagateLegacyHeader bool) {
	if req.Operation.Name != "Publish" {
		return
	}
	...
	msgAttr := &sns.MessageAttributeValue{
		...
		StringValue: aws.String(athttp.FormatTraceparentHeader(traceContext)),
	}
	...
	input.MessageAttributes[athttp.W3CTraceparentHeader] = msgAttr
	if propagateLegacyHeader {
		input.MessageAttributes[athttp.AtatusTraceparentHeader] = msgAttr
	}
	if tracestate != "" {
		input.MessageAttributes[athttp.TracestateHeader] = &sns.MessageAttributeValue{
			DataType:    aws.String("String"),
