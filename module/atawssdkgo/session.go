...
package atawssdkgo // import "go.atatus.com/agent/module/atawssdkgo"
...
func WrapSession(s *session.Session) *session.Session {
	s.Handlers.Build.PushFrontNamed(request.NamedHandler{
		Name: "go.atatus.com/agent/module/atawssdkgo/build",
		Fn:   build,
	})
	s.Handlers.Send.PushFrontNamed(request.NamedHandler{
		Name: "go.atatus.com/agent/module/atawssdkgo/send",
		Fn:   send,
	})
	s.Handlers.Complete.PushBackNamed(request.NamedHandler{
		Name: "go.atatus.com/agent/module/atawssdkgo/complete",
		Fn:   complete,
	})
...
func build(req *request.Request) {
	...
	span := tx.StartSpan("", spanType, atatus.SpanFromContext(ctx))
	if !span.Dropped() {
		ctx = atatus.ContextWithSpan(ctx, span)
...
func send(req *request.Request) {
	...
	if !span.Dropped() {
		ctx = atatus.ContextWithSpan(ctx, span)
		req.HTTPRequest = athttp.RequestWithContext(ctx, req.HTTPRequest)
	...
	if region := req.Config.Region; region != nil {
		span.Context.SetDestinationCloud(atatus.DestinationCloudSpanContext{
			Region: *region,
		})
	}
