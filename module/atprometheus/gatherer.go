...
package atprometheus // import "go.atatus.com/agent/module/atprometheus"
...
// Wrap returns an atatus.MetricsGatherer wrapping g.
func Wrap(g prometheus.Gatherer) atatus.MetricsGatherer {
	return gatherer{g}
}
...
func (g gatherer) GatherMetrics(ctx context.Context, out *atatus.Metrics) error {
...
func makeLabels(lps []*dto.LabelPair) []atatus.MetricLabel {
	labels := make([]atatus.MetricLabel, len(lps))
	for i, lp := range lps {
		labels[i] = atatus.MetricLabel{Name: lp.GetName(), Value: lp.GetValue()}
	}
	return labels
}
