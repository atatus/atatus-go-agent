...
type requestTracer struct {
	...
	TLS,
	Request,
	Response *atatus.Span

	mu       sync.Mutex
	ended    bool
	Connects map[connectKey]*atatus.Span
}

func withClientTrace(ctx context.Context, tx *atatus.Transaction, parent *atatus.Span) (context.Context, *requestTracer) {
	r := requestTracer{
		Connects: make(map[connectKey]*atatus.Span),
	}
