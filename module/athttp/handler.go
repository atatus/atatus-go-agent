...
// a transaction to Atatus.
//
// By default, the returned Handler will use atatus.DefaultTracer.
...
// h.Tracer, or atatus.DefaultTracer if h.Tracer is nil.
...
func StartTransaction(tracer *atatus.Tracer, name string, req *http.Request) (*atatus.Transaction, *http.Request) {
	traceContext, ok := getRequestTraceparent(req, W3CTraceparentHeader)
	if !ok {
		traceContext, ok = getRequestTraceparent(req, AtatusTraceparentHeader)
	}
	if ok {
	...
	ctx := atatus.ContextWithTransaction(req.Context(), tx)
...
func StartTransactionWithBody(tracer *atatus.Tracer, name string, req *http.Request) (*atatus.Transaction, *atatus.BodyCapturer, *http.Request) {
	tx, req := StartTransaction(tracer, name, req)
	bc := tracer.CaptureHTTPRequestBody(req)
	if bc != nil {
		req = RequestWithContext(atatus.ContextWithBodyCapturer(req.Context(), bc), req)
	}
	return tx, bc, req
}

func getRequestTraceparent(req *http.Request, header string) (atatus.TraceContext, bool) {
...
func SetTransactionContext(tx *atatus.Transaction, req *http.Request, resp *Response, body *atatus.BodyCapturer) {
...
func SetContext(ctx *atatus.Context, req *http.Request, resp *Response, body *atatus.BodyCapturer) {
