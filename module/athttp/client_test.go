...
func TestClientTraceContextHeaders(t *testing.T) {
	t.Run("with-atatus-apm-traceparent", func(t *testing.T) {
		testClientTraceContextHeaders(t, "Atatus-Apm-Traceparent", "Traceparent")
	})
	t.Run("without-atatus-apm-traceparent", func(t *testing.T) {
		os.Setenv("ATATUS_USE_TRACEPARENT_HEADER", "true")
...
func testClientTraceContextHeaders(t *testing.T, traceparentHeaders ...string) {
	...
	tx := tracer.StartTransactionOptions("name", "type", atatus.TransactionOptions{
		TraceContext: atatus.TraceContext{
			...
			Options: atatus.TraceOptions(0).WithRecorded(true),
			State:   atatus.NewTraceState(atatus.TraceStateEntry{Key: "vendor", Value: "tracestate"}),
		},
	})
	ctx := atatus.ContextWithTransaction(context.Background(), tx)
	...
	traceparentValue := athttp.FormatTraceparentHeader(atatus.TraceContext{
		Trace:   atatus.TraceID(span.TraceID),
		Span:    atatus.SpanID(span.ID),
		Options: atatus.TraceOptions(0).WithRecorded(true),
	})
...
func TestClientSpanDropped(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		w.Write([]byte(req.Header.Get("Atatus-Apm-Traceparent")))
	}))
	defer server.Close()
	...
	clientTraceContext, err := athttp.ParseTraceparentHeader(string(responseBodies[0]))
	require.NoError(t, err)
	...
	clientTraceContext, err = athttp.ParseTraceparentHeader(string(responseBodies[1]))
	require.NoError(t, err)
...
func TestClientTransactionUnsampled(t *testing.T) {
	...
	tracer, transport := transporttest.NewRecorderTracer()
	defer tracer.Close()
	tracer.SetSampler(atatus.NewRatioSampler(0)) // sample nothing
	...
	clientTraceContext, err := athttp.ParseTraceparentHeader(string(responseBody))
	require.NoError(t, err)
	assert.Equal(t, transaction.TraceID, model.TraceID(clientTraceContext.Trace))
	assert.Equal(t, transaction.ID, model.SpanID(clientTraceContext.Span))
}
...
func TestClientError(t *testing.T) {
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		client := athttp.WrapClient(http.DefaultClient)
...
func TestClientCancelRequest(t *testing.T) {
	...
	apmtest.WithTransaction(func(ctx context.Context) {
		client := &http.Client{
			Transport: athttp.WrapRoundTripper(transport),
...
func TestWithClientRequestName(t *testing.T) {
	...
	option := athttp.WithClientRequestName(func(_ *http.Request) string {
		return "http://test"
	})
...
func TestWithClientTrace(t *testing.T) {
	...
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		mustGET(ctx, server.URL, athttp.WithClientTrace())
	})
...
func TestWithClientSpanType(t *testing.T) {
	...
	option := athttp.WithClientSpanType("cache")
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		mustGET(ctx, server.URL, option)
	})
...
func mustGET(ctx context.Context, url string, o ...athttp.ClientOption) (statusCode int, responseBody string) {
	client := athttp.WrapClient(http.DefaultClient, o...)
