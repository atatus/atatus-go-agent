...
const (
	...
	// for AtatusTraceparentHeader, but the more specific constants
	// below should be preferred. In a future version this will be
	// replaced by the standard W3C header.
	TraceparentHeader = AtatusTraceparentHeader

	// AtatusTraceparentHeader is the legacy HTTP header for trace propagation,
	// maintained for backwards compatibility with older agents.
	AtatusTraceparentHeader = "Atatus-Apm-Traceparent"
...
// traceparent header.
func FormatTraceparentHeader(c atatus.TraceContext) string {
	const version = 0
...
func ParseTraceparentHeader(h string) (atatus.TraceContext, error) {
	var out atatus.TraceContext
	if len(h) < 3 || h[2] != '-' {
	...
	case 0:
		...
		out.Options = atatus.TraceOptions(traceOptions[0])
		return out, nil
	}
}
...
func ParseTracestateHeader(h ...string) (atatus.TraceState, error) {
	var entries []atatus.TraceStateEntry
	for _, h := range h {
		for {
			...
			if equal == -1 {
				return atatus.TraceState{}, errors.New("missing '=' in tracestate entry")
			}
			entries = append(entries, atatus.TraceStateEntry{Key: kv[:equal], Value: kv[equal+1:]})
		}
	}
	return atatus.NewTraceState(entries...), nil
}
