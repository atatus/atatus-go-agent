...
package atgoredis // import "go.atatus.com/agent/module/atgoredis"
...
// Wrap wraps client such that executed commands are reported as spans to Atatus,
...
func processPipeline(ctx context.Context) func(oldProcess func(cmds []redis.Cmder) error) func(cmds []redis.Cmder) error {
	return func(oldProcess func(cmds []redis.Cmder) error) func(cmds []redis.Cmder) error {
		return func(cmds []redis.Cmder) error {
			pipelineSpan, ctx := atatus.StartSpan(ctx, "(pipeline)", "db.redis")
			...
			for i := len(cmds); i > 0; i-- {
				...
				span, _ := atatus.StartSpan(ctx, cmdName, "db.redis")
				defer span.End()
			}
