...
var (
	unitTestCases = []struct {
		...
		{
			clientTypeBase,
			atgoredis.Wrap(redisEmptyClient()),
		},
		{
			clientTypeBase,
			atgoredis.Wrap(redisEmptyClient()).WithContext(context.Background()),
		},
		{
			clientTypeCluster,
		...
		{
			clientTypeCluster,
			atgoredis.Wrap(redisEmptyClusterClient()),
		},
		{
			clientTypeCluster,
			atgoredis.Wrap(redisEmptyClusterClient()).WithContext(context.Background()),
		},
		{
			clientTypeRing,
			redisEmptyRing(),
		...
		{
			clientTypeRing,
			atgoredis.Wrap(redisEmptyRing()),
		},
		{
			clientTypeRing,
			atgoredis.Wrap(redisEmptyRing()).WithContext(context.Background()),
		},
	}
)
	...
	for i, testCase := range unitTestCases {
		t.Run(fmt.Sprintf("test %d", i), func(t *testing.T) {
			...
			_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
				client := atgoredis.Wrap(client).WithContext(ctx)
				client.Ping()
			})
...
func TestWithContext(t *testing.T) {
	for i, testCase := range unitTestCases {
		t.Run(fmt.Sprintf("test %d", i), func(t *testing.T) {
			...
			ping := func(ctx context.Context, client atgoredis.Client) {
				span, ctx := atatus.StartSpan(ctx, "ping", "custom")
				defer span.End()
