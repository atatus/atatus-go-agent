	...
	for i, testCase := range testCases {
		t.Run(fmt.Sprintf("test %d", i), func(t *testing.T) {
			...
			_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
				for i := 0; i < 2; i++ {
					wrappedClient := atgoredis.Wrap(client).WithContext(ctx)
...
func cleanRedis(t *testing.T, client redis.UniversalClient, clientType int) {
	...
	case clientTypeCluster:
		...
		case atgoredis.Client:
			err = client.(atgoredis.Client).Cluster().ForEachMaster(func(master *redis.Client) error {
				return master.FlushDB().Err()
			})
		}
	...
	case clientTypeRing:
		...
		case atgoredis.Client:
			err = client.(atgoredis.Client).RingClient().ForEachShard(func(shard *redis.Client) error {
				return shard.FlushDB().Err()
			})
		}
...
func getTestCases(t *testing.T) []struct {
	...
	return []struct {
		...
		{
			clientTypeBase,
			atgoredis.Wrap(redisClient(t)),
		},
		{
			clientTypeBase,
			atgoredis.Wrap(redisClient(t)).WithContext(context.Background()),
		},
		{
			clientTypeCluster,
		...
		{
			clientTypeCluster,
			atgoredis.Wrap(redisClusterClient(t)),
		},
		{
			clientTypeCluster,
			atgoredis.Wrap(redisClusterClient(t)).WithContext(context.Background()),
		},
		{
			clientTypeRing,
			redisRingClient(t),
		...
		{
			clientTypeRing,
			atgoredis.Wrap(redisRingClient(t)),
		},
		{
			clientTypeRing,
			atgoredis.Wrap(redisRingClient(t)).WithContext(context.Background()),
		},
	}
}
