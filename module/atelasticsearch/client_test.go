...
func TestWrapRoundTripper(t *testing.T) {
	...
	client := &http.Client{Transport: atelasticsearch.WrapRoundTripper(http.DefaultTransport)}
...
func TestSpanDuration(t *testing.T) {
	...
	client := &http.Client{Transport: atelasticsearch.WrapRoundTripper(nil)}
	_, spans, errs := apmtest.WithTransaction(func(ctx context.Context) {
		before := time.Now()
...
func TestStatementBodyReadError(t *testing.T) {
	...
	client := &http.Client{Transport: atelasticsearch.WrapRoundTripper(roundTripper)}
...
func TestDestination(t *testing.T) {
	...
	client := &http.Client{Transport: atelasticsearch.WrapRoundTripper(rt)}
...
func TestTraceHeaders(t *testing.T) {
	...
	assert.Contains(t, headers, athttp.AtatusTraceparentHeader)
	assert.Contains(t, headers, athttp.W3CTraceparentHeader)
	assert.Contains(t, headers, athttp.TracestateHeader)
}
...
func TestClientTransactionUnsampled(t *testing.T) {
	...
	clientTraceContext, err := athttp.ParseTraceparentHeader(string(body))
	require.NoError(t, err)
	assert.Equal(t, transaction.TraceID, model.TraceID(clientTraceContext.Trace))
	assert.Equal(t, transaction.ID, model.SpanID(clientTraceContext.Span))
}
