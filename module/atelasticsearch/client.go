...
package atelasticsearch // import "go.atatus.com/agent/module/atelasticsearch"
...
// request as a span to Atatus, if the request's context contains a
// sampled transaction.
//
...
func (r *roundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if !tx.Sampled() {
		athttp.SetHeaders(req, traceContext, false)
		return r.r.RoundTrip(req)
	}

	propagateLegacyHeader := tx.ShouldPropagateLegacyHeader()
	name := requestName(req)
	span := tx.StartSpan(name, "db.elasticsearch", atatus.SpanFromContext(ctx))

	if span.Dropped() {
		span.End()
		athttp.SetHeaders(req, traceContext, propagateLegacyHeader)
		return r.r.RoundTrip(req)
	}
