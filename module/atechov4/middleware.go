...
package atechov4 // import "go.atatus.com/agent/module/atechov4"
...
func (m *middleware) handle(c echo.Context) error {
	...
	tx, body, req := athttp.StartTransactionWithBody(m.tracer, name, req)
	defer tx.End()
	c.SetRequest(req)
	...
	defer func() {
		...
		tx.Result = athttp.StatusCodeResult(resp.Status)
		if tx.Sampled() {
	...
	if handlerErr != nil {
		...
		if handlerErr, ok := handlerErr.(*echo.HTTPError); ok {
			...
			if c.Path() == reqPath {
				...
				if unknownRoute {
					tx.Name = athttp.UnknownRouteRequestName(req)
				}
			}
		}
...
func setContext(ctx *atatus.Context, req *http.Request, resp *echo.Response, body *atatus.BodyCapturer) {
