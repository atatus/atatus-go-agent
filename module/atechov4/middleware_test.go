...
import (
	...
	atecho "go.atatus.com/agent/module/atechov4"
...
func TestMiddlewareHTTPSuite(t *testing.T) {
	...
	e := echo.New()
	e.Use(atecho.Middleware(atecho.WithTracer(tracer)))
	...
	e.POST("/explicit_error_capture", func(c echo.Context) error {
		...
		e := atatus.CaptureError(c.Request().Context(), errors.New("total explosion"))
		e.Send()
...
func doRequest(e *echo.Echo, method, url string) *httptest.ResponseRecorder {
	...
	req.Header.Set("User-Agent", "atecho_test")
