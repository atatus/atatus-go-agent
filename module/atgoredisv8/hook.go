...
package atgoredisv8 // import "go.atatus.com/agent/module/atgoredisv8"
...
// hook is an implementation of redis.Hook that reports cmds as spans to Atatus.
type hook struct{}

// NewHook returns a redis.Hook that reports cmds as spans to Atatus.
...
func (r *hook) BeforeProcess(ctx context.Context, cmd redis.Cmder) (context.Context, error) {
	_, ctx = atatus.StartSpanOptions(ctx, getCmdName(cmd), "db.redis", atatus.SpanOptions{
		ExitSpan: true,
	})
	return ctx, nil
}
...
func (r *hook) BeforeProcessPipeline(ctx context.Context, cmds []redis.Cmder) (context.Context, error) {
	...
	_, ctx = atatus.StartSpanOptions(ctx, cmdNameBuf.String(), "db.redis", atatus.SpanOptions{
		ExitSpan: true,
	})
	return ctx, nil
}
