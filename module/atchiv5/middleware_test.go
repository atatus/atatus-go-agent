...
import (
	...
	"go.atatus.com/agent/module/atchiv5"
...
func TestMiddleware(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(atchiv5.Middleware(atchiv5.WithTracer(tracer.Tracer)))
...
func TestMiddleware_With(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Route("/prefix", func(r chi.Router) {
		r.With(atchiv5.Middleware(atchiv5.WithTracer(tracer.Tracer))).Get("/articles/{category}/{id}", articleHandler)
	})
...
func TestWithRequestIgnorer(t *testing.T) {
	cases := []struct {
		name    string
		ignorer athttp.RequestIgnorerFunc
		expect  bool
	}{
		{
			"nil-ignorer",
		...
		{
			"athttp.NewRegexpRequestIgnorer",
			athttp.NewRegexpRequestIgnorer(regexp.MustCompile(".*")),
			false,
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := chi.NewRouter()
			r.Use(atchiv5.Middleware(
				atchiv5.WithTracer(tracer.Tracer),
				atchiv5.WithRequestIgnorer(tt.ignorer),
			))
			r.Route("/prefix", func(r chi.Router) {
				r.Get("/articles/{category}/{id}", articleHandler)
			})
