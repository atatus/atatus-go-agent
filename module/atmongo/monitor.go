...
package atmongo // import "go.atatus.com/agent/module/atmongo"
...
func init() {
	atatus.RegisterTypeErrorDetailer(
		reflect.TypeOf(mongo.CommandError{}),
		atatus.ErrorDetailerFunc(func(err error, details *atatus.ErrorDetails) {
...
func CommandMonitor(opts ...Option) *event.CommandMonitor {
	cm := commandMonitor{
		...
		spans:        make(map[commandKey]*atatus.Span),
	}
	for _, o := range opts {
		o(&cm)
	}
...
type commandMonitor struct {
	...
	mu    sync.Mutex
	spans map[commandKey]*atatus.Span
}
...
func (c *commandMonitor) started(ctx context.Context, event *event.CommandStartedEvent) {
	...
	span, _ := atatus.StartSpan(ctx, spanName, "db.mongodb.query")
	if span.Dropped() {
		return
	}
