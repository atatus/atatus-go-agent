...
func logKV(tracer *atatus.Tracer, tx *atatus.Transaction, span *atatus.Span, time time.Time, keyValues []interface{}) {
	var ctx logContext
...
func logFields(tracer *atatus.Tracer, tx *atatus.Transaction, span *atatus.Span, time time.Time, fields []log.Field) {
	var ctx logContext
...
func (c *logContext) emit(tracer *atatus.Tracer, tx *atatus.Transaction, span *atatus.Span, time time.Time) {
	if !c.errorEvent {
		return
	}
	...
	e := tracer.NewErrorLog(atatus.ErrorLogRecord{Message: c.message, Error: c.err})
	if !time.IsZero() {
		e.Timestamp = time
	}
	if span != nil {
