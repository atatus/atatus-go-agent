...
// Span returns s.span, the underlying atatus.Span. This is used to satisfy
// SpanFromContext.
func (s *otSpan) Span() *atatus.Span {
	return s.span
}
...
func (s *otSpan) setSpanContext() {
	var (
		dbContext       atatus.DatabaseSpanContext
		component       string
		httpURL         string
		httpMethod      string
		httpHost        string
...
func (s *otSpan) setTransactionContext() {
	...
	if s.ctx.tx.Result == "" {
		if httpStatusCode != -1 {
			s.ctx.tx.Result = athttp.StatusCodeResult(httpStatusCode)
...
// We send error events to Atatus.
