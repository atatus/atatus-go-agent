...
import (
	...
	"go.atatus.com/agent/module/atot"
...
func TestStartSpanFromContextMixed(t *testing.T) {
	...
	apmSpan1, ctx := atatus.StartSpan(ctx, "apm1", "apm")
	otSpan1, ctx := opentracing.StartSpanFromContext(ctx, "ot1")
	apmSpan2, ctx := atatus.StartSpan(ctx, "apm2", "apm")
...
func testTraceInjectExtract(t *testing.T, shouldPropagateLegacyHeader bool) {
	os.Setenv("ATATUS_USE_TRACEPARENT_HEADER", strconv.FormatBool(shouldPropagateLegacyHeader))
	...
	if shouldPropagateLegacyHeader {
		assert.Contains(t, headers1, "Atatus-Apm-Traceparent")
	} else {
		assert.NotContains(t, headers1, "Atatus-Apm-Traceparent")
	}
}
...
func BenchmarkSpanSetSpanContext(b *testing.B) {
	...
	tracer := atot.New(atot.WithTracer(apmtest.DiscardTracer))
	parentSpan := tracer.StartSpan("parent")
	for i := 0; i < b.N; i++ {
...
func newTestTracer() (opentracing.Tracer, *atatus.Tracer, *transporttest.RecorderTransport) {
	apmtracer, recorder := transporttest.NewRecorderTracer()
	tracer := atot.New(atot.WithTracer(apmtracer))
