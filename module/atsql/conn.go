...
package atsql // import "go.atatus.com/agent/module/atsql"
...
func (c *conn) startStmtSpan(ctx context.Context, stmt, spanType string) (*atatus.Span, context.Context) {
	return c.startSpan(ctx, c.driver.querySignature(stmt), spanType, stmt)
}

func (c *conn) startSpan(ctx context.Context, name, spanType, stmt string) (*atatus.Span, context.Context) {
	span, ctx := atatus.StartSpan(ctx, name, spanType)
	if !span.Dropped() {
...
func (c *conn) finishSpan(ctx context.Context, span *atatus.Span, result *driver.Result, resultError *error) {
	...
	default:
		if e := atatus.CaptureError(ctx, *resultError); e != nil {
			e.Send()
		}
	}
	span.End()
