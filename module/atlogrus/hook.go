...
package atlogrus // import "go.atatus.com/agent/module/atlogrus"
...
func (h *Hook) tracer() *atatus.Tracer {
	tracer := h.Tracer
	if tracer == nil {
...
func (h *Hook) Fire(entry *logrus.Entry) error {
	...
	// Extract trace context added with atlogrus.TraceContext,
	// and include it in the reported error.
	if traceID, ok := entry.Data[FieldKeyTraceID].(atatus.TraceID); ok {
		errlog.TraceID = traceID
	}
	if transactionID, ok := entry.Data[FieldKeyTransactionID].(atatus.SpanID); ok {
		errlog.TransactionID = transactionID
		errlog.ParentID = transactionID
	}
	if spanID, ok := entry.Data[FieldKeySpanID].(atatus.SpanID); ok {
