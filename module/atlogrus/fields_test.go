...
func TestTraceContext(t *testing.T) {
	...
	tx, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		...
		defer span.End()
		logger.WithTime(time.Unix(0, 0).UTC()).WithFields(atlogrus.TraceContext(ctx)).Debug("beep")
	})
...
func TestTraceContextEmpty(t *testing.T) {
	...
	// atlogrus.TraceContext will return nil if the context does not contain a transaction.
