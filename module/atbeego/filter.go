...
package atbeego // import "go.atatus.com/agent/module/atbeego"
...
func Middleware(o ...Option) func(http.Handler) http.Handler {
	...
	return func(h http.Handler) http.Handler {
		return athttp.Wrap(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			if tx != nil {
				...
				req = athttp.RequestWithContext(ctx, req)
			}
			h.ServeHTTP(w, req)
		}), athttp.WithTracer(opts.tracer), athttp.WithServerRequestName(athttp.UnknownRouteRequestName))
	}
}
...
// WrapRecoverFunc updates config's RecoverFunc so that panics will be reported to Atatus
...
func WrapRecoverFunc(config *beego.Config) {
	...
	config.RecoverFunc = func(context *beegocontext.Context) {
		if tx := atatus.TransactionFromContext(context.Request.Context()); tx == nil {
			orig(context)
		}
	}
}
...
func setTransactionContext(tx *atatus.Transaction, state *beegoFilterState) {
