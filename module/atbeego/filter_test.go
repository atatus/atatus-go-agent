...
import (
	...
	"go.atatus.com/agent/module/atbeego"
...
func TestMiddleware(t *testing.T) {
	...
	atbeego.AddFilters(handlers)

	tracer, transport := transporttest.NewRecorderTracer()
	server := httptest.NewServer(
		atbeego.Middleware(atbeego.WithTracer(tracer))(handlers),
	)
	defer server.Close()
...
func (c *testController) Get() {
	span, _ := atatus.StartSpan(c.Ctx.Request.Context(), "testController.Get", "controller")
	defer span.End()
