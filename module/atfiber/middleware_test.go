...
func TestMiddlewareMultipleSameHandler(t *testing.T) {
	...
	do := func(url, method, targetTransactionName string) {
		...
		app := fiber.New()
		app.Use(atfiber.Middleware(atfiber.WithTracer(tracer.Tracer)))
		...
		req.Header.Set("User-Agent", "atfiber_test")
...
func TestMiddleware(t *testing.T) {
	...
	e := fiber.New()
	e.Use(atfiber.Middleware(atfiber.WithTracer(tracer.Tracer)))
...
func TestMiddlewarePanicPropagation(t *testing.T) {
	...
	e.Use(
		...
		atfiber.Middleware(
			atfiber.WithTracer(tracer.Tracer),
			atfiber.WithPanicPropagation(),
		),
	)
	e.Get("/panic", handlePanic)
...
func TestMiddlewareRequestIgnorer(t *testing.T) {
	...
	e.Use(
		atfiber.Middleware(
			atfiber.WithTracer(tracer.Tracer),
			atfiber.WithRequestIgnorer(func(ctx *fasthttp.RequestCtx) bool {
