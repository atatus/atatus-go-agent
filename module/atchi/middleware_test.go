...
import (
	...
	"go.atatus.com/agent/module/atchi"
...
func TestMiddleware(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(atchi.Middleware(atchi.WithTracer(tracer.Tracer)))
...
func TestMiddleware_With(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Route("/prefix", func(r chi.Router) {
		r.With(atchi.Middleware(atchi.WithTracer(tracer.Tracer))).Get("/articles/{category}/{id}", articleHandler)
	})
...
func TestWithRequestIgnorer(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := chi.NewRouter()
			r.Use(atchi.Middleware(
				atchi.WithTracer(tracer.Tracer),
				atchi.WithRequestIgnorer(tt.ignorer),
			))
			r.Route("/prefix", func(r chi.Router) {
				r.Get("/articles/{category}/{id}", articleHandler)
			})
