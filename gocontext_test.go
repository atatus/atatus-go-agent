	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			...
			for i := 0; i < 1000; i++ {
				tx := tracer.StartTransaction("name", "type")
				ctx := atatus.ContextWithTransaction(context.Background(), tx)
				span1, ctx := atatus.StartSpan(ctx, "name", "type")
				span1.End()
				atatus.CaptureError(ctx, errors.New("boom")).Send()
...
func TestContextStartSpanOptions(t *testing.T) {
	...
	tx, spans, _ := apmtest.WithTransactionOptions(atatus.TransactionOptions{
		Start: txTimestamp,
	}, func(ctx context.Context) {
		span0, ctx := atatus.StartSpanOptions(ctx, "span0", "type", atatus.SpanOptions{
		...
		span1, ctx := atatus.StartSpanOptions(ctx, "span1", "type", atatus.SpanOptions{})
		...
		span2, ctx := atatus.StartSpanOptions(ctx, "span2", "type", atatus.SpanOptions{
		...
		span3, ctx := atatus.StartSpanOptions(ctx, "span3", "type", atatus.SpanOptions{})
...
func TestDetachedContext(t *testing.T) {
	funcB := func(ctx context.Context) chan chan error {
		...
		go func() {
			ch := <-chch
			defer close(ch)
			span, ctx := atatus.StartSpan(ctx, "funcB", "custom")
			defer span.End()
			ch <- ctx.Err()
		}()
		return chch
	...
	funcA := func(ctx context.Context) chan chan error {
		span, ctx := atatus.StartSpan(ctx, "funcA", "custom")
		defer span.End()
		return funcB(atatus.DetachedContext(ctx))
	}
