...
func TestStartSpanTransactionNotSampled(t *testing.T) {
	tracer, _ := atatus.NewTracer("tracer_testing", "")
	defer tracer.Close()
	// sample nothing
	tracer.SetSampler(atatus.NewRatioSampler(0))
...
func TestSpanParentID(t *testing.T) {
	...
	assert.NotEqual(t, parentID, atatus.SpanID{})
...
func TestSpanTiming(t *testing.T) {
	...
	tx, spans, _ := apmtest.WithTransactionOptions(
		atatus.TransactionOptions{Start: txStart},
		func(ctx context.Context) {
			time.Sleep(500 * time.Millisecond)
			span, _ := atatus.StartSpan(ctx, "name", "type")
			spanStart = time.Now()
			time.Sleep(500 * time.Millisecond)
			spanEnd = time.Now()
			span.End()
...
func TestSpanType(t *testing.T) {
	...
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		for _, spanType := range spanTypes {
			span, _ := atatus.StartSpan(ctx, "name", spanType)
			span.End()
		}
	})
...
func TestStartExitSpan(t *testing.T) {
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		span, _ := atatus.StartSpanOptions(ctx, "name", "type", atatus.SpanOptions{ExitSpan: true})
	...
	tx := tracer.StartTransaction("name", "type")
	span := tx.StartSpanOptions("name", "type", atatus.SpanOptions{ExitSpan: true})
	assert.True(t, span.IsExitSpan())
...
func TestCompressSpanNonSiblings(t *testing.T) {
	...
	for _, span := range createSpans {
		span := tx.StartSpanOptions(span.name, span.typ, atatus.SpanOptions{
...
func TestCompressSpanExactMatch(t *testing.T) {
	...
	tests := []struct {
		...
		//                                                                        r
		{
			...
			setup: func(*testing.T) func() {
				// This setting
				envVarName := "ATATUS_SPAN_COMPRESSION_EXACT_MATCH_MAX_DURATION"
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			currentTime := txStart
			for i := 0; i < 10; i++ {
				span := tx.StartSpanOptions("SELECT * FROM users", "mysql", atatus.SpanOptions{
			...
			{
				span := tx.StartSpanOptions("GET /", "request", atatus.SpanOptions{
					ExitSpan: true, Start: currentTime,
				})
				span.Duration = 100 * time.Nanosecond
				currentTime = currentTime.Add(span.Duration)
			...
			}
			{
				...
				span := tx.StartSpanOptions("calculate complex", "internal", atatus.SpanOptions{
					Start: currentTime,
				})
				span.Duration = 100 * time.Nanosecond
				currentTime = currentTime.Add(span.Duration)
...
func TestCompressSpanSameKind(t *testing.T) {
	...
	testCase := func(tracer *apmtest.RecordingTracer) (model.Transaction, []model.Span, func()) {
		...
		for i := 0; i < 5; i++ {
			span := tx.StartSpanOptions(fmt.Sprint("GET ", path[i]), "request", atatus.SpanOptions{
				ExitSpan: true, Start: currentTime,
			})
			span.Duration = 100 * time.Nanosecond
			currentTime = currentTime.Add(span.Duration)
		...
		{
			span := tx.StartSpanOptions("GET /f", "request", atatus.SpanOptions{
				ExitSpan: true, Start: currentTime,
			})
	...
	t.Run("10msThreshold", func(t *testing.T) {
		...
		os.Setenv("ATATUS_SPAN_COMPRESSION_SAME_KIND_MAX_DURATION", "10ms")
		defer os.Unsetenv("ATATUS_SPAN_COMPRESSION_SAME_KIND_MAX_DURATION")
...
func TestCompressSpanSameKindParentSpan(t *testing.T) {
	...
	currentTime := txStart
	{
		...
		parent, ctx := atatus.StartSpanOptions(ctx, "internal op", "internal", atatus.SpanOptions{
			Start: currentTime,
		})
		// Have span propagate context downstream, this should not allow for
		// compression
		child, ctx := atatus.StartSpanOptions(ctx, "GET /resource", "request", atatus.SpanOptions{
			Start: currentTime.Add(100 * time.Microsecond),
		})

		grandChild, _ := atatus.StartSpanOptions(ctx, "GET /different", "request", atatus.SpanOptions{
			ExitSpan: true,
	...
	}
	{
		...
		parent, ctx := atatus.StartSpanOptions(ctx, "another op", "internal", atatus.SpanOptions{
			Start: currentTime.Add(50 * time.Microsecond),
		})
		child, _ := atatus.StartSpanOptions(ctx, "GET /res", "request", atatus.SpanOptions{
			ExitSpan: true,
			Start:    currentTime.Add(120 * time.Microsecond),
		})

		otherChild, _ := atatus.StartSpanOptions(ctx, "GET /diff", "request", atatus.SpanOptions{
			ExitSpan: true,
...
func TestCompressSpanSameKindParentSpanContext(t *testing.T) {
	...
	parent, ctx := atatus.StartSpanOptions(ctx, "parent", "internal", atatus.SpanOptions{
		Start: parentStart,
	})
	...
	for i := 0; i < 3; i++ {
		span, _ := atatus.StartSpanOptions(ctx, "db", "redis", atatus.SpanOptions{
			ExitSpan: true,
			Start:    childrenStart,
		})
	...
	subParent, ctx := atatus.StartSpanOptions(ctx, "algorithm", "internal", atatus.SpanOptions{
		Start: childrenStart.Add(time.Millisecond),
	})
	childrenStart = childrenStart.Add(time.Millisecond)
	for _, childCfg := range testSpans {
		child, _ := atatus.StartSpanOptions(ctx, childCfg.name, childCfg.typ, atatus.SpanOptions{
			ExitSpan: true,
...
func TestCompressSpanSameKindConcurrent(t *testing.T) {
	...
	tx := tracer.StartTransaction("name", "type")
	ctx := atatus.ContextWithTransaction(context.Background(), tx)
	parent, ctx := atatus.StartSpan(ctx, "parent", "internal")
	...
	for i := 0; i < count; i++ {
		go func(i int) {
			child, _ := atatus.StartSpanOptions(ctx, fmt.Sprint(i), "request", atatus.SpanOptions{
				ExitSpan: true,
			})
			spanStarted <- struct{}{}
			child.End()
...
func TestCompressSpanPrematureEnd(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run("ParentContext/"+test.name, func(t *testing.T) {
			...
			for i := 0; i < 4; i++ {
				child, _ := atatus.StartSpanOptions(ctx, fmt.Sprint(i), "type", atatus.SpanOptions{
	...
	t.Run("ParentFromTx", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			child := tx.StartSpanOptions(fmt.Sprint(i), "type", atatus.SpanOptions{
				Parent:   parent.TraceContext(),
				ExitSpan: true,
			})
			child.Duration = time.Microsecond
...
func TestExitSpanDoesNotOverwriteDestinationServiceResource(t *testing.T) {
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		span, _ := atatus.StartSpanOptions(ctx, "name", "type", atatus.SpanOptions{ExitSpan: true})
		assert.True(t, span.IsExitSpan())
		span.Context.SetDestinationService(atatus.DestinationServiceSpanContext{
...
func TestTracerStartSpanIDSpecified(t *testing.T) {
	spanID := atatus.SpanID{0, 1, 2, 3, 4, 5, 6, 7}
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		span, _ := atatus.StartSpanOptions(ctx, "name", "type", atatus.SpanOptions{SpanID: spanID})
		span.End()
	})
	require.Len(t, spans, 1)
...
func TestSpanSampleRate(t *testing.T) {
	tracer := apmtest.NewRecordingTracer()
	defer tracer.Close()
	tracer.SetSampler(atatus.NewRatioSampler(0.55555))

	tx := tracer.StartTransactionOptions("name", "type", atatus.TransactionOptions{
		// Use a known transaction ID for deterministic sampling.
		TransactionID: atatus.SpanID{1, 2, 3, 4, 5, 6, 7, 8},
	})
...
func TestSpanFastExit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() func() {
				os.Setenv("ATATUS_EXIT_SPAN_MIN_DURATION", "2ms")
				return func() { os.Unsetenv("ATATUS_EXIT_SPAN_MIN_DURATION") }
			},
			expect: expect{
				spans:                  1,
				droppedSpansStatsCount: 0,
		...
		},
		{
			...
			setup: func() func() {
				os.Setenv("ATATUS_EXIT_SPAN_MIN_DURATION", "3ms")
				return func() { os.Unsetenv("ATATUS_EXIT_SPAN_MIN_DURATION") }
			},
			expect: expect{
				spans:                  0,
		...
		},
		{
			...
			setup: func() func() {
				os.Setenv("ATATUS_EXIT_SPAN_MIN_DURATION", "100us")
				return func() { os.Unsetenv("ATATUS_EXIT_SPAN_MIN_DURATION") }
			},
			expect: expect{
				spans:                  1,
...
func TestSpanFastExitWithCompress(t *testing.T) {
	...
	for i := 0; i < 499; i++ {
		span, _ := atatus.StartSpanOptions(ctx, "compressed", "type", atatus.SpanOptions{
	...
	// outcome is "failure".
	errorSpan, _ := atatus.StartSpanOptions(ctx, "compressed", "type", atatus.SpanOptions{
		ExitSpan: true, Start: ts,
	})
	...
	for i := 0; i < 100; i++ {
		span, _ := atatus.StartSpanOptions(ctx, "compressed", "anothertype", atatus.SpanOptions{
			ExitSpan: true, Start: ts,
		})
		span.Duration = time.Millisecond
		ts = ts.Add(span.Duration)
	...
	for i := 0; i < 100; i++ {
		span, _ := atatus.StartSpanOptions(ctx, fmt.Sprint(i), fmt.Sprint(i), atatus.SpanOptions{
			ExitSpan: true, Start: ts,
		})
...
func TestSpanFastExitNoTransaction(t *testing.T) {
	...
	tx := tracer.StartTransaction("name", "type")
	ctx := atatus.ContextWithTransaction(context.Background(), tx)
	span, _ := atatus.StartSpanOptions(ctx, "compressed", "type", atatus.SpanOptions{ExitSpan: true})
...
func TestSpanOutcome(t *testing.T) {
	_, spans, _ := apmtest.WithTransaction(func(ctx context.Context) {
		span1, _ := atatus.StartSpan(ctx, "name", "type")
		span1.End()

		span2, _ := atatus.StartSpan(ctx, "name", "type")
		span2.Outcome = "unknown"
		span2.End()

		span3, _ := atatus.StartSpan(ctx, "name", "type")
		span3.Context.SetHTTPStatusCode(200)
		span3.End()

		span4, _ := atatus.StartSpan(ctx, "name", "type")
		span4.Context.SetHTTPStatusCode(400)
		span4.End()

		span5, ctx := atatus.StartSpan(ctx, "name", "type")
