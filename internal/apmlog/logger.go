...
// ATATUS_LOG_FILE and ATATUS_LOG_LEVEL.
...
func (l *LevelLogger) logf(level Level, format string, args ...interface{}) {
	...
	// at_logging
	fmt.Fprintf(l.w, "[atatus-go-agent] [%+v] [%+v] %s\n", time.Now().Format("2006-01-02T15:04:05.000Z"), level.String(), fmt.Sprintf(format, args...))
	// l.w.Write(level.String(), time.Now(), fmt.Sprintf(format, args...))
