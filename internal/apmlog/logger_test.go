...
func init() {
	os.Unsetenv("ATATUS_LOG_FILE")
	os.Unsetenv("ATATUS_LOG_LEVEL")
	DefaultLogger = nil
}
...
func TestInitDefaultLoggerInvalidFile(t *testing.T) {
	...
	DefaultLogger = nil
	os.Setenv("ATATUS_LOG_FILE", ".")
	defer os.Unsetenv("ATATUS_LOG_FILE")
	InitDefaultLogger()
...
func TestInitDefaultLoggerFile(t *testing.T) {
	...
	DefaultLogger = nil
	os.Setenv("ATATUS_LOG_FILE", filepath.Join(dir, "log.json"))
	defer os.Unsetenv("ATATUS_LOG_FILE")
	InitDefaultLogger()
...
func TestInitDefaultLoggerStdio(t *testing.T) {
	...
	for _, filename := range []string{"stdout", "stderr"} {
		DefaultLogger = nil
		os.Setenv("ATATUS_LOG_FILE", filename)
		InitDefaultLogger()
		require.NotNil(t, DefaultLogger)
...
func TestInitDefaultLoggerInvalidLevel(t *testing.T) {
	...
	DefaultLogger = nil
	os.Setenv("ATATUS_LOG_FILE", filepath.Join(dir, "log.json"))
	os.Setenv("ATATUS_LOG_LEVEL", "panic")
	defer os.Unsetenv("ATATUS_LOG_FILE")
	defer os.Unsetenv("ATATUS_LOG_LEVEL")
	InitDefaultLogger()
	...
	require.NoError(t, err)
	assert.Regexp(t, `{"level":"error","time":".*","message":"error message"}`, string(data))
	assert.Regexp(t, `invalid ATATUS_LOG_LEVEL "panic", falling back to "error"`, logbuf.String())
}
...
func TestInitDefaultLoggerLevel(t *testing.T) {
	...
	DefaultLogger = nil
	os.Setenv("ATATUS_LOG_FILE", filepath.Join(dir, "log.json"))
	os.Setenv("ATATUS_LOG_LEVEL", "debug")
	defer os.Unsetenv("ATATUS_LOG_FILE")
	defer os.Unsetenv("ATATUS_LOG_LEVEL")
	InitDefaultLogger()
