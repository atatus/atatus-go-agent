...
func (c *featureContext) initTestSuite(s *godog.TestSuiteContext) {
	s.BeforeSuite(func() {
		c.httpServer = httptest.NewServer(
			athttp.Wrap(c.httpHandler, athttp.WithTracer(c.tracer.Tracer)),
		)

		c.grpcServer = grpc.NewServer(grpc.UnaryInterceptor(
			atgrpc.NewUnaryServerInterceptor(
				atgrpc.WithTracer(c.tracer.Tracer),
				atgrpc.WithRecovery(),
			),
		))
...
func (c *featureContext) initScenario(s *godog.ScenarioContext) {
	...
	s.Step("cloud metadata is collected", func() error {
		_, _, _, cloud, _, err := getSubprocessMetadata(append([]string{
			"ATATUS_CLOUD_PROVIDER=auto", // Explicitly enable cloud metadata detection
...
func (c *featureContext) checkAuthorizationHeader(expected string) error {
	...
	os.Setenv("ATATUS_SECRET_TOKEN", c.secretToken)
	os.Setenv("ATATUS_API_KEY", c.apiKey)
	os.Setenv("ATATUS_SERVER_URL", server.URL)
	if _, err := transport.InitDefault(); err != nil {
		return err
	}
	tracer, err := atatus.NewTracer("godog", "")
	if err != nil {
		return err
	}
	defer tracer.Close()
...
func (c *featureContext) sendHTTPRequest(ctx context.Context) error {
	client := athttp.WrapClient(c.httpServer.Client())
