...
func TestParseDurationEnv(t *testing.T) {
	const envKey = "ATATUS_TEST_DURATION"
	os.Unsetenv(envKey)
	...
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: invalid unit in duration 1us (allowed units: ms, s, m)")
	...
	_, err = configutil.ParseDurationEnv(envKey, 42*time.Second)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: invalid character ' ' in duration 5 h")

	os.Setenv(envKey, "5h")
	_, err = configutil.ParseDurationEnv(envKey, 42*time.Second)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: invalid unit in duration 5h (allowed units: ms, s, m)")

	os.Setenv(envKey, "5")
	_, err = configutil.ParseDurationEnv(envKey, 42*time.Second)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: missing unit in duration 5 (allowed units: ms, s, m)")

	os.Setenv(envKey, "blah")
	_, err = configutil.ParseDurationEnv(envKey, 42*time.Second)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: invalid duration blah")
}
...
func TestParseDurationOptionsEnv(t *testing.T) {
	...
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_DURATION: invalid unit in duration 1ns (allowed units: us, ms, s, m)")
	assert.Equal(t, 10*time.Microsecond, d)
...
func TestParseSizeEnv(t *testing.T) {
	...
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_SIZE: invalid character ' ' in size 5 mb")

	os.Setenv(envKey, "5tb")
	_, err = configutil.ParseSizeEnv(envKey, 0)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_SIZE: invalid unit in size 5tb (allowed units: B, KB, MB, GB)")

	os.Setenv(envKey, "5")
	_, err = configutil.ParseSizeEnv(envKey, 0)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_SIZE: missing unit in size 5 (allowed units: B, KB, MB, GB)")

	os.Setenv(envKey, "blah")
	_, err = configutil.ParseSizeEnv(envKey, 0)
	assert.EqualError(t, err, "failed to parse ATATUS_TEST_SIZE: invalid size blah")
}

func TestParseBoolEnv(t *testing.T) {
	const envKey = "ATATUS_TEST_BOOL"
	os.Unsetenv(envKey)
	defer os.Unsetenv(envKey)
	...
	assert.EqualError(t, err, `failed to parse ATATUS_TEST_BOOL: strconv.ParseBool: parsing "falsk": invalid syntax`)
}

func TestParseListEnv(t *testing.T) {
	const envKey = "ATATUS_TEST_LIST"
	os.Unsetenv(envKey)
	defer os.Unsetenv(envKey)
...
func TestParseWildcardPatternsEnv(t *testing.T) {
	const envKey = "ATATUS_TEST_WILDCARDS"
	os.Unsetenv(envKey)
	defer os.Unsetenv(envKey)
