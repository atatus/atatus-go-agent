...
func TestTracerMetricsGatherer(t *testing.T) {
	...
	tracer.RegisterMetricsGatherer(atatus.GatherMetricsFunc(
		func(ctx context.Context, m *atatus.Metrics) error {
			m.Add("http.request", []atatus.MetricLabel{
				{Name: "code", Value: "400"},
				{Name: "path", Value: "/"},
			}, 3)
			m.Add("http.request", []atatus.MetricLabel{
...
func TestTracerDisableMetrics(t *testing.T) {
	os.Setenv("ATATUS_DISABLE_METRICS", "golang.heap.*, system.memory.*, system.process.*")
	defer os.Unsetenv("ATATUS_DISABLE_METRICS")
