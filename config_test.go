...
func TestTracerCentralConfigUpdate(t *testing.T) {
	...
	t.Run("log_level", func(t *testing.T) {
		...
		logfile := filepath.Join(tempdir, "atatus.log")
...
func testTracerCentralConfigUpdate(t *testing.T, logger atatus.Logger, serverResponse string, isRemote func(*apmtest.RecordingTracer) bool) {
	...
	tracer.Tracer, err = atatus.NewTracerOptions(atatus.TracerOptions{Transport: &testTransport})
	require.NoError(t, err)
...
func TestTracerCentralConfigUpdateDisabled(t *testing.T) {
	...
	os.Setenv("ATATUS_CENTRAL_CONFIG", "false")
	defer os.Unsetenv("ATATUS_CENTRAL_CONFIG")

	httpTransport, err := transport.NewHTTPTransport()
	require.NoError(t, err)
	tracer, err := atatus.NewTracerOptions(atatus.TracerOptions{Transport: httpTransport})
	require.NoError(t, err)
	defer tracer.Close()
...
func TestTracerConfigWatcherPrecedence(t *testing.T) {
	...
	tracer.SetSampler(atatus.NewRatioSampler(1))
	sampled := tracer.StartTransaction("name", "type").Sampled()
