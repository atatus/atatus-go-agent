...
import (
	...
	"go.atatus.com/agent/internal/apmhostutil"
...
func TestTracerStats(t *testing.T) {
	...
	tracer.Flush(nil)
	assert.Equal(t, atatus.TracerStats{
		TransactionsSent: 500,
	}, tracer.Stats())
}

func TestTracerClosedSendNonBlocking(t *testing.T) {
	tracer, err := atatus.NewTracer("tracer_testing", "")
...
func capturePanic(tracer *atatus.Tracer, v interface{}) {
	tx := tracer.StartTransaction("name", "type")
	defer tx.End()
	span := tx.StartSpan("name", "type", nil)
	defer span.End()
...
func TestTracerRequestSize(t *testing.T) {
	os.Setenv("ATATUS_API_REQUEST_SIZE", "1KB")
	...
	os.Setenv("ATATUS_API_REQUEST_TIME", "60s")
...
func TestTracerBufferSize(t *testing.T) {
	os.Setenv("ATATUS_API_REQUEST_SIZE", "1KB")
	os.Setenv("ATATUS_API_BUFFER_SIZE", "10KB")
	defer os.Unsetenv("ATATUS_API_REQUEST_SIZE")
...
func TestTracerMetadata(t *testing.T) {
	...
	assert.Equal(t, "none", os.Getenv("ATATUS_CLOUD_PROVIDER"))
