...
import (
	...
	"go.atatus.com/agent/internal/pkgerrorsutil"
...
func (e *Error) SetTransaction(tx *Transaction) {
	...
	tx.mu.RUnlock()
	e.setSpanData(traceContext, traceContext.Span, txName, txType, custom)
}
...
func (e *Error) SetSpan(s *Span) {
	...
	e.setSpanData(s.traceContext, s.transactionID, txName, txType, custom)
}
...
// Send enqueues the error for sending to the Atatus server.
//
