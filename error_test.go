...
func TestErrorID(t *testing.T) {
	var errorID atatus.ErrorID
	_, _, errors := apmtest.WithTransaction(func(ctx context.Context) {
		e := atatus.CaptureError(ctx, errors.New("boom"))
		errorID = e.ID
		e.Send()
	})
...
func TestErrorsStackTrace(t *testing.T) {
	...
	assert.Equal(t, "go.atatus.com/agent_test", exception.Module)
...
func TestErrorsStackTraceLimit(t *testing.T) {
	...
	const n = 2
	for i := -1; i < n; i++ {
		os.Setenv("ATATUS_STACK_TRACE_LIMIT", strconv.Itoa(i))
		modelError := sendError(t, &errorsStackTracer{
...
func TestRuntimeStackTrace(t *testing.T) {
	...
	frame := stacktrace[0]
	assert.Equal(t, "go.atatus.com/agent_test", frame.Module)
...
func TestCaptureErrorNoTransaction(t *testing.T) {
	...
	e := atatus.CaptureError(context.Background(), errors.New("boom"))
...
func TestErrorLogRecord(t *testing.T) {
	...
	error_ := tracer.NewErrorLog(atatus.ErrorLogRecord{
		Message: "log-message",
...
func TestErrorCauserInterface(t *testing.T) {
	...
	var e Causer = atatus.CaptureError(context.Background(), errors.New("boom"))
...
func TestErrorErrorInterface(t *testing.T) {
	var e error = atatus.CaptureError(context.Background(), errors.New("boom"))
...
func TestErrorTransactionSampled(t *testing.T) {
	_, _, errors := apmtest.WithTransaction(func(ctx context.Context) {
		atatus.TransactionFromContext(ctx).Type = "foo"
		atatus.CaptureError(ctx, errors.New("boom")).Send()
...
func TestErrorTransactionCustomContext(t *testing.T) {
	...
	_, ctx = atatus.StartSpan(ctx, "foo", "bar")
	atatus.CaptureError(ctx, errors.New("boom")).Send()
...
func TestErrorDetailer(t *testing.T) {
	...
	atatus.RegisterTypeErrorDetailer(reflect.TypeOf(error1{}), atatus.ErrorDetailerFunc(func(err error, details *atatus.ErrorDetails) {
		details.SetAttr("a", "error1")
	}))

	type error2 struct{ error }
	atatus.RegisterTypeErrorDetailer(reflect.TypeOf(&error2{}), atatus.ErrorDetailerFunc(func(err error, details *atatus.ErrorDetails) {
		details.SetAttr("b", "*error2")
	}))

	atatus.RegisterErrorDetailer(atatus.ErrorDetailerFunc(func(err error, details *atatus.ErrorDetails) {
	...
	_, _, errs := apmtest.WithTransaction(func(ctx context.Context) {
		atatus.CaptureError(ctx, error1{errors.New("error1")}).Send()
		atatus.CaptureError(ctx, &error2{errors.New("error2")}).Send()
	})
...
func TestStdlibErrorDetailers(t *testing.T) {
	t.Run("syscall.Errno", func(t *testing.T) {
		_, _, errs := apmtest.WithTransaction(func(ctx context.Context) {
			atatus.CaptureError(ctx, syscall.Errno(syscall.EAGAIN)).Send()
		})
	...
	test := func(err error, expectedAttrs map[string]interface{}) {
		t.Run(fmt.Sprintf("%T", err), func(t *testing.T) {
			_, _, errs := apmtest.WithTransaction(func(ctx context.Context) {
				atatus.CaptureError(ctx, err).Send()
			})
			require.Len(t, errs, 1)
...
func sendError(t *testing.T, err error, f ...func(*atatus.Error)) model.Error {
	tracer, r := transporttest.NewRecorderTracer()
	defer tracer.Close()
